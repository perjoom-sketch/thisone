...
function getRedisConfig() {
  const url = limitString(process.env.UPSTASH_REDIS_REST_URL, 1000).replace(/\/$/, '');
  const token = limitString(process.env.UPSTASH_REDIS_REST_TOKEN, 1000);
...
async function redisPipeline(commands, config = getRedisConfig()) {
...
async function storeRedisAggregate(event) {
  await redisPipeline(buildRedisWriteCommands(event));
}
...
async function storeAnalyticsEvent(event) {
  ...
  if (redisConfig) {
    try {
      await storeRedisAggregate(safeEvent);
