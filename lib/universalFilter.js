...
3. 질문 적합도(queryFit)와 액세서리일 확률(accessoryProbability) 평가
4. 애매하면 보수적으로 판단
5. 반드시 JSON만 출력
...
- 비정상적으로 저가인 상품은 액세서리 가능성을 강하게 의심
- 애매하면 keep=false
- id는 그대로 유지
...
function genericFallback(query, items) {
  ...
  const results = items.map((item) => {
    ...
    const keep = targetRole === 'main_product'
      ? !looksAccessory && !looksSibling && !priceCheck.hardReject
      : looksAccessory && !looksSibling;
    ...
    return {
      ...
      accessoryProbability: looksAccessory ? 0.92 : (priceCheck.suspicious ? 0.72 : 0.12),
      ambiguity: looksSibling ? 0.55 : 0.25,
      reason: keep
        ? 'fallback 통과'
        : looksAccessory
          ? 'fallback에서 액세서리로 판단'
