...
// CommonJS
// 원칙: 애매하면 살린다. 명확한 액세서리/부품/렌탈/판촉/무관 후보만 제거한다.
...
function genericFallback(query, items) {
  ...
  const results = items.map((item) => {
    ...
    const looksRentalPromo = rentalPromoWords.some(word => name.includes(word));
    const looksSibling = siblingWords.some(word => name.includes(word));

    let keep = true;

    if (guaranteedMain && !clearlyMaskNonRetail) {
      keep = true;
