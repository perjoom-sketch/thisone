...
function buildResultSignalDiagnostics(result) {
  ...
  const negativeKeywords = listKeywordHits(text, NEGATIVE_KEYWORDS);
  return {
    ...result,
    matchedPositiveKeywords: positiveKeywords,
    matchedNegativeKeywords: negativeKeywords,
    positiveKeywordCount: positiveKeywords.length,
    negativeKeywordCount: negativeKeywords.length,
    signalScore: positiveKeywords.length - negativeKeywords.length,
    classification: classifyKeywordBalance(positiveKeywords.length, negativeKeywords.length)
  };
}
  ...
  return (results || [])
    .map((result, index) => {
      ...
      return {
        ...
        matchedPositiveKeywords: positiveKeywords,
        matchedNegativeKeywords: negativeKeywords,
        weightedPositiveHits: Number((weight * positiveKeywords.length).toFixed(2)),
        weightedNegativeHits: Number((weight * negativeKeywords.length).toFixed(2)),
        classification: classifyKeywordBalance(positiveKeywords.length, negativeKeywords.length)
      };
    })
    .filter(Boolean);
