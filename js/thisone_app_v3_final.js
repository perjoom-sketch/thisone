...
function renderRecentSearches() {
  ...
  const matchedRecentSearches = getMatchedRecentSearches(inputValue);
  matchedRecentSearches.forEach((query) => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'recent-search-item';
...
function getMatchedRecentSearches(inputValue = '') {
  const keyword = String(inputValue || '').trim();
  if (!keyword) return RecentSearchUIState.searches;
  return RecentSearchUIState.searches.filter((item) => {
    if (!item) return false;
    return item.startsWith(keyword) || item.includes(keyword);
  });
}
