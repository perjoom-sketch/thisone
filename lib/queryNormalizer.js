...
function normalizeHouseholdQuery(query){
  ...
  if(/(화장지|휴지)/.test(q)){
    ...
    if(/3겹/.test(q)&&/30m/i.test(q)&&/30롤/.test(q)){q='3겹 화장지 30m 30롤';}
  }
...
function improveQuery(originalQuery){
  ...
  // 쇼핑 조건 수식어 제거 (네이버 검색에 방해되는 키워드)
  const conditionWords = [
    '무료배송', '배송비 포함', '배송비포함', '공식몰', '공식 몰',
    '가장 나은', '가장 좋은', '최저가', '유지비 포함',
    '할인', '특가', '세일'
  ];
  ...
  if(q.includes('유모차')||q.includes('맘카페')||q.includes('유아차')){
    q=q.replace(/맘카페 반응 좋은|맘카페 추천|좋은/g,'');
    q='유모차 '+q.trim();
    if(originalQuery.includes('맘카페')){q='유모차 추천 '+q.replace('유모차 ','');}
  }
  q=q.replace(/-[^\s]+/g,'');
...
module.exports={normalizeHouseholdQuery,improveQuery};
