...
function getReferenceMedianPrice(items) {
  const plausibleMainPrices = (items || [])
    .filter(item => {
      const s = getGenericSemanticSignals(item.name);
      return !s.looksAccessory && !s.looksDecorToy && !s.looksSibling && !s.looksBundle;
    })
    .map(numericPrice)
    .filter(v => v > 0)
    .sort((a, b) => a - b);
...
function buildPrompt(query, items) {
  ...
  const medianPrice = getReferenceMedianPrice(items);
...
function genericFallback(query, items) {
  ...
  const results = items.map((item) => {
    ...
    return {
      id: item.id,
      keep,
      itemRole: signals.looksAccessory || signals.looksBundle ? 'accessory' : 'main_product',
...
async function applyUniversalAIFilter({ query, items }) {
  ...
  const targetRole = detectTargetRole(query);
  const medianPrice = getReferenceMedianPrice(sliced);
