...
// 2. 기존 callAnthropic 함수를 이 함수로 완전히 교체
...
// 3. applyUniversalAIFilter 함수 내부에서 호출부 변경
// const parsed = await callAnthropic(prompt); -> 이 부분을 아래로 변경
...
function getGenericSemanticSignals(name) {
  ...
  return {
    ...
    looksBundle: bundleWords.some(word => text.includes(word)) || hasCountPack,
    looksToyLike: false
  };
}
...
function getCategorySignals(category, name) {
  ...
  if (category === 'stroller') {
    const strollerAccessory = [
      '컵홀더', '모기장', '레인커버', '방풍커버', '풋머프', '시트', '라이너',
      '클립', '집게', '유모차클립', '고정클립', '멀티클립', '스트랩',
      '고리', '후크', '가방걸이', '운전대', '핸들토이', '핸들 장난감',
      '깜빡이', '사운드토이', '장착', '부착'
    ];
    ...
    const strollerDecor = [
      '모형', '미니어처', '소품', '장식', '방꾸미기', '포토존',
      '스튜디오', '촬영', '오브제', '인테리어'
    ];

    const strollerToyLike = [
      '운전대', '핸들', '깜빡이', '자동차소리', '사운드', '키즈카페',
      '생일선물', '놀이', '역할놀이', '아기장난감', '토이'
    ];

    return {
      looksAccessory: strollerAccessory.some(word => text.includes(word)),
      looksSibling: strollerSibling.some(word => text.includes(word)),
      looksDecorToy: strollerDecor.some(word => text.includes(word)),
      looksToyLike: strollerToyLike.some(word => text.includes(word)),
      looksBundle: /\b\d+\s*(p|개|입|종|pcs)\b/i.test(text) || text.includes('세트') || text.includes('묶음')
    };
  }

  if (category === 'air_purifier') {
    const accessory = [
      '필터', '헤파필터', '교체용', '리필', '커버', '거치대', '받침대'
    ];
    const sibling = ['가습기', '제습기', '선풍기', '에어서큘레이터'];
    return {
      looksAccessory: accessory.some(word => text.includes(word)),
      looksSibling: sibling.some(word => text.includes(word)),
      looksDecorToy: false,
      looksToyLike: false,
  ...
  if (category === 'printer') {
    const accessory = [
      '토너', '잉크', '카트리지', '드럼', '용지', '케이블', '리필', '교체용'
    ];
    const sibling = ['스캐너', '복합기 부품'];
    return {
      looksAccessory: accessory.some(word => text.includes(word)),
      looksSibling: sibling.some(word => text.includes(word)),
      looksDecorToy: false,
  ...
  if (category === 'fan') {
    const accessory = ['날개', '리모컨', '커버', '브라켓', '덕트', '거치대'];
    const sibling = ['에어컨', '공기청정기', '제습기'];
    return {
      looksAccessory: accessory.some(word => text.includes(word)),
      looksSibling: sibling.some(word => text.includes(word)),
      looksDecorToy: false,
...
function getReferenceMedianPrice(items, category) {
  ...
  if (!plausibleMainPrices.length) {
    return getMedianPrice(items);
  }
...
- 클립, 집게, 스트랩, 홀더, 커버, 모기장, 4P/세트/묶음 형태의 장착형 부속품은 본품(main_product)이 아닙니다
- 질문이 본품 요청이면 이런 장착형 부속품은 keep=false 쪽으로 판단하세요
- 특히 stroller 카테고리에서 운전대, 핸들토이, 깜빡이, 자동차소리, 키즈카페, 생일선물 같은 표현은 장난감성/놀이성 부속품일 가능성이 높습니다
- 사용자가 액세서리를 찾으면 본품은 keep=false 쪽으로 판단
- 같은 계열이 아니면 keep=false 쪽으로 판단
- 비정상적으로 저가인 상품은 액세서리/모형/장난감성 부속품 가능성을 강하게 의심
- 애매하면 keep=false
- id는 그대로 유지

가격 참고:
- 현재 후보군 기준 중앙값(reference median price): ${medianPrice ? `${medianPrice}원` : '알 수 없음'}
...
출력 형식:
{
  "targetRole": "${detectTargetRole(query)}",
  "results": [
    {
      "id": "1",
      "keep": true,
...
async function applyUniversalAIFilter({ query, items }) {
  ...
  try {
    ...
    const filteredItems = sliced.filter((item) => {
      const hard = deterministicHardReject(item, category, resolvedTargetRole, medianPrice);
    ...
    const rejectedItems = sliced
      ...
      .map((item) => {
        ...
        let reason = hard.reject ? hard.reason : (r.reason || '판단 불충분');
