...
function decideKeep(targetRole, r, priceCheck, item, query) {
  ...
  if (isMaskQuery(q) && isMaskMainProductText(itemText) && !isClearlyMaskNonRetailText(itemText) && itemPrice >= MASK_PRICE_FLOOR) {
    return true;
  }
...
async function applyUniversalAIFilter({ query, items }) {
  ...
  try {
    ...
    const filteredItems = sliced.filter((item) => {
      ...
      if (!r) return !priceCheck.hardReject; // AI가 판단 못해도 hardReject 후보는 제거한다
