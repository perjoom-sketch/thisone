...
function genericFallback(query, items) {
  ...
  const isAmbiguous = isAmbiguousQuery(query, items);
  ...
  const results = items.map((item) => {
    ...
    const categoryRole = getCategoryRole(item);

    if (categoryRole !== 'unknown') {
      let keep = true;
      let reason = 'category_match';

      if (categoryRole === 'irrelevant' && !isAmbiguous) {
        keep = false;
        reason = 'irrelevant_category';
      } else if (categoryRole === 'accessory' && userIntent === 'main' && !isAmbiguous) {
        keep = false;
        reason = 'accessory_main_intent';
      } else if (categoryRole === 'main' && userIntent === 'accessory' && !isAmbiguous) {
        keep = false;
        reason = 'main_accessory_intent';
      }

      return {
        id: item.id,
        keep,
        itemRole: categoryRole === 'accessory' ? 'accessory' : 'main_product',
        relationType: categoryRole === 'irrelevant' ? 'unrelated' : 'same_family',
        queryFit: keep ? 0.7 : 0.25,
        textMatch: keep ? 0.75 : 0.3,
        textImageConsistency: 0.5,
        accessoryProbability: categoryRole === 'accessory' ? 0.92 : (priceCheck.suspicious ? 0.35 : 0.12),
        ambiguity: isAmbiguous ? 0.6 : (keep ? 0.35 : 0.6),
        reason
      };
    }

    if (categoryRole === 'unknown') {
      // legacy text parsing fallback: categoryMap 미커버 상품군과 애매 검색어 안전장치.
      // 화이트리스트 본품 확정
...
function decideKeep(targetRole, r, priceCheck, item, query, allItems = []) {
  ...
  if (role === 'unknown') {
    // legacy text parsing fallback: categoryMap이 커버하지 못한 후보만 기존 안전장치로 판단한다.
    ...
    if (targetRole === 'main_product') {
      const fallbackRole = r.itemRole || 'unknown';
      ...
      if (fallbackRole === 'accessory' && accessoryProbability >= 0.85 && queryFit < 0.4) return false;

      // unrelated 기준 완화: queryFit < 0.3 이하일 때만 제거 (기존 0.45 → 0.3)
      if (relation === 'unrelated' && queryFit < 0.3) return false;

      // 나머지는 살린다
      if (fallbackRole === 'unknown') return true;
