...
function buildPrompt(query, items, category, medianPrice, targetRole) {
  return `
당신은 베테랑 쇼핑 큐레이터이자 '최저가 낚시'를 잡아내는 전문가입니다.
후보 상품들이 본품(main_product)인지, 아니면 낚시용 액세서리/소품인지 전문가의 안목으로 판단하세요.
...
판단 가이드라인:
1. 가격 불일치: 중앙가 대비 가격이 너무 낮으면(30% 이하), 제목에 '본품'이라 써있어도 'accessory'로 의심하세요.
2. 제목 키워드: "추가금", "단품", "호환", "부품", "전용", "리필" 등의 단어가 있으면 액세서리일 확률이 90%입니다.
3. 이미지 불일치: 제목은 세트인데 가격이 너무 싸면 액세서리입니다.
...
async function applyUniversalAIFilter({ query, items }) {
  ...
  const sliced = items.slice(0, 18).map(item => ({
    id: String(item.id),
  ...
  try {
    ...
    let filteredItems = items.filter(item => {
      ...
      // 전문가가 분석한 가격 리스크와 실제 가격 비율을 종합 판단
      if (targetRole === "main_product") {
        if (r.itemRole !== "main_product") return false;
