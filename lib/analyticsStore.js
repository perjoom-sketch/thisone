...
function buildRedisWriteCommands(event) {
  ...
  const commands = [
    ...
    ['INCR', dayKey(date, `eventName:${event.eventName}:${internalSuffix}`)],
    ['INCR', dayKey(date, `mode:${mode}`)],
    ['INCR', dayKey(date, `mode:${mode}:${internalSuffix}`)],
  ...
  if (event.eventName === 'page_view') {
    commands.push(
      ['INCR', dayKey(date, 'pageViews')],
      ['INCR', dayKey(date, `pageViews:${internalSuffix}`)]
    );
...
function addPeriod(target, source) {
  const sourceEvents = toCount(source.events || source.totalEvents);
  target.events += sourceEvents;
  target.totalEvents += sourceEvents;
  ...
  target.externalPageViews += toCount(source.externalPageViews);
  target.internalPageViews += toCount(source.internalPageViews);
...
function sortBreakdownMap(map, labelKey) {
  return Array.from(map.entries())
    .map(([label, counts]) => ({
      [labelKey]: label,
      count: toCount(counts?.count),
      externalCount: toCount(counts?.externalCount),
      internalCount: toCount(counts?.internalCount)
    }))
...
async function readAnalyticsSummary() {
  ...
  dates.forEach((date) => {
    readCommands.push(
      ...
      ['GET', dayKey(date, 'pageViews:external')],
      ['GET', dayKey(date, 'pageViews:internal')],
  ...
  dates.forEach((date, index) => {
    ...
    const events = toCount(results[offset]);
    daily.push({
      date,
      events,
      totalEvents: events,
      ...
      externalPageViews: toCount(results[offset + 4]),
      internalPageViews: toCount(results[offset + 5]),
      visitors: toCount(results[offset + 6]),
      externalVisitors: toCount(results[offset + 7]),
      internalVisitors: toCount(results[offset + 8])
    });
    modesByDate.set(date, Array.isArray(results[offset + 9]) ? results[offset + 9].map((mode) => limitString(mode, 40)).filter(Boolean) : []);
    eventNamesByDate.set(date, Array.isArray(results[offset + 10]) ? results[offset + 10].map((eventName) => limitString(eventName, 60)).filter(Boolean) : []);
  });
  ...
  dates.forEach((date) => {
    modesByDate.get(date).forEach((mode) => {
      ...
      breakdownCommands.push(
        ['GET', dayKey(date, `mode:${mode}`)],
        ['GET', dayKey(date, `mode:${mode}:external`)],
        ['GET', dayKey(date, `mode:${mode}:internal`)]
      );
    });
    eventNamesByDate.get(date).forEach((eventName) => {
      breakdownRefs.push({ type: 'eventName', label: eventName });
      breakdownCommands.push(
        ['GET', dayKey(date, `eventName:${eventName}`)],
        ['GET', dayKey(date, `eventName:${eventName}:external`)],
        ['GET', dayKey(date, `eventName:${eventName}:internal`)]
      );
    });
  });
  ...
  breakdownRefs.forEach((ref, index) => {
    ...
    const count = toCount(breakdownResults[resultOffset]);
    const externalCount = toCount(breakdownResults[resultOffset + 1]);
    const internalCount = toCount(breakdownResults[resultOffset + 2]);
    const map = ref.type === 'mode' ? byModeMap : byEventNameMap;
    const current = map.get(ref.label) || { count: 0, externalCount: 0, internalCount: 0 };
    map.set(ref.label, {
      count: current.count + count,
      externalCount: current.externalCount + externalCount,
      internalCount: current.internalCount + internalCount
    });
  });
