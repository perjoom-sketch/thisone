...
function isEvidenceLimitedSourceQuality(sourceQuality) {
  return sourceQuality !== 'good';
}

const PROGRAM_OR_RECRUITMENT_KEYWORDS = [
  '모집', '선발', '채용', '공고', '사업', '프로그램', '운영', '수행기관', '위촉', '용역', '민간',
  '참여', '안내', '활동', '지킴이', '서포터즈', '점검', '컨설팅'
];

const DIRECT_LEGAL_STATUS_KEYWORDS = [
  '공무원', '정식 직원', '정규직', '소속 직원', '근로감독관', '행정처분', '법적 신분',
...
function sourcesSuggestProgramOrRecruitmentOnly(sources) {
  if (!Array.isArray(sources) || sources.length === 0) return false;
  const combined = sources.map(sourceText).join(' ');
  const hasProgramNotice = hasAnyKeyword(combined, PROGRAM_OR_RECRUITMENT_KEYWORDS);
  const hasDirectStatus = hasAnyKeyword(combined, DIRECT_LEGAL_STATUS_KEYWORDS);
  return hasProgramNotice && !hasDirectStatus;
}

function needsEvidenceLimitedOfficialPhrasing(analysis, researchPlan, sources) {
  const sourceQuality = researchPlan?.sourceQuality || 'none';
  return Boolean(
    analysis?.needsOfficialSource
    && (isEvidenceLimitedSourceQuality(sourceQuality) || sourcesSuggestProgramOrRecruitmentOnly(sources))
  );
}
...
- 빠진 주의사항
- 아직 확인되지 않은 부분
- 출처가 사업·모집·공고만 확인하고 법적 신분을 직접 확인하지 못하는데 초안이 “아니다/정식 직원이 아니다/공무원이 아니다”라고 단정했는지

반드시 가능한 한 JSON만 반환하세요. 링크나 출처를 새로 만들지 마세요. 검토 메모를 사실처럼 확정하지 마세요. 확인된 사실은 공개 출처에서 온 내용만 인정하고, 합리적 해석은 해석으로 남겨야 합니다.
...
const REVIEW_SECTION_LABELS = ['결론', '확인된 내용', '합리적 해석', '확인되지 않은 부분', '지금 확인할 것', '근거', '이유', '지금 할 일', '주의할 점'];
const AWKWARD_REVIEW_PHRASES = [
  '결론 아닙니다',
  '확인된 내용은 확인되었습니다',
  '합리적 해석은 합리적입니다'
];

function removeSectionHeadingLines(text) {
  return String(text || '')
    .split('\n')
    .filter((line) => !new RegExp(`^\\s*(?:\\d+\\.\\s*)?(?:${REVIEW_SECTION_LABELS.join('|')})\\s*[:：]?$`).test(line.trim()))
    .join('\n');
}

function stripRepeatedSectionLabel(text) {
  let clean = normalizeText(text, 600);
  for (const label of REVIEW_SECTION_LABELS) {
    clean = clean.replace(new RegExp(`^${label}\\s*(?:은|는|입니다|:|：|-|–|—)?\\s*`, 'i'), '').trim();
  }
  return clean;
}

function cleanReviewedSectionText(text, maxLength = 320) {
  let clean = removeSectionHeadingLines(text)
    .replace(/\[[^\]]+\]\([^)]*\)/g, '')
    .replace(/https?:\/\/\S+/gi, '')
    .replace(/\s+/g, ' ')
    .trim();

  for (const phrase of AWKWARD_REVIEW_PHRASES) {
    clean = clean.replace(new RegExp(`${phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[.!?。！？]?`, 'g'), '').trim();
  }

  clean = stripRepeatedSectionLabel(clean)
    .replace(/^아닙니다[.!?。！？\s]*/i, '')
    .replace(/\s*[.。]\s*[.。]?\s*$/, (match) => (match.trim().length > 1 ? '.' : match))
    .trim();

  return normalizeText(clean, maxLength);
}

function firstUsefulSentence(text) {
  const clean = cleanReviewedSectionText(text, 800);
  const sentence = clean.split(/(?<=[.!?。！？다요함임됨음])\s+/).find((part) => {
    const candidate = cleanReviewedSectionText(part, 320);
    return candidate.length >= 8 && !AWKWARD_REVIEW_PHRASES.some((phrase) => candidate.includes(phrase));
  }) || clean;
  return cleanReviewedSectionText(sentence, 320) || '현재 확인 가능한 범위 안에서만 조심스럽게 답해야 합니다.';
}

function isAffiliationOrAuthorityQuestion(analysis) {
  return analysis?.taskType === 'affiliation/status' || analysis?.taskType === 'authority/role';
}

function fallbackConclusionForAnalysis(analysis, weakSources) {
  if (!isAffiliationOrAuthorityQuestion(analysis)) {
    return weakSources
      ? '공개 출처만으로는 단정하기 어려워, 확인된 내용과 미확인 부분을 나눠 봐야 합니다.'
      : '확인된 공개 근거를 기준으로만 조심스럽게 판단해야 합니다.';
  }

  const institution = analysis?.institutionWords?.[0] || '해당 기관';
  if (institution.includes('고용노동부') || institution.includes('노동부')) {
    return weakSources
      ? '공개 출처만으로는 고용노동부 정식 직원이라고 확인하기 어렵습니다.'
      : '정식 고용노동부 직원이라고 단정하기는 어렵습니다.';
  }

  return weakSources
    ? `공개 출처만으로는 ${institution} 정식 소속이라고 확인하기 어렵습니다.`
    : `${institution} 정식 소속이라고 단정하기는 어렵습니다.`;
}

function buildReviewedConclusion(draftAnswer, analysis, weakSources) {
  const conclusion = firstUsefulSentence(draftAnswer);
  const rawDraft = String(draftAnswer || '');
  const rawAwkward = AWKWARD_REVIEW_PHRASES.some((phrase) => rawDraft.includes(phrase));
  const awkward = AWKWARD_REVIEW_PHRASES.some((phrase) => conclusion.includes(phrase));
  if (awkward || /^아닙니다\b/i.test(conclusion) || (rawAwkward && conclusion === '현재 확인 가능한 범위 안에서만 조심스럽게 답해야 합니다.')) {
    return fallbackConclusionForAnalysis(analysis, weakSources);
  }
  return conclusion;
}

function buildReasonableInterpretation(draftAnswer, conclusion, analysis) {
  if (isAffiliationOrAuthorityQuestion(analysis)) {
    return '제도나 공고가 확인되더라도, 그 사실만으로 해당 인력의 법적 신분, 공무원 여부, 행정처분 권한까지 확정할 수는 없습니다.';
  }

  const sentence = firstUsefulSentence(draftAnswer);
  if (sentence === conclusion) {
    return '확인된 근거가 말하는 범위까지만 해석하고, 출처가 직접 말하지 않는 부분은 가능성으로 남겨야 합니다.';
  }
  return sentence;
}

function sourceSummaryBullets(sources, sourceQuality, analysis) {
  if (!Array.isArray(sources) || sources.length === 0) {
    return ['현재 답변에 직접 근거로 삼을 공개 출처를 충분히 확인하지 못했습니다.'];
  }

  const prefix = isEvidenceLimitedSourceQuality(sourceQuality)
  ...
  if (isAffiliationOrAuthorityQuestion(analysis)) {
    bullets.push('- 확인된 것은 공개 출처에 나온 사업·공고·안내 범위이며, 그 자체가 개인의 정식 소속이나 공무원 신분을 증명하는 것은 아닙니다.');
  }
...
function affiliationCheckList(analysis) {
  ...
  return [
    ...
    '명함의 소속기관과 운영기관명',
    '위촉장 또는 위탁/수행기관 표시',
    '근로감독관 여부와 행정처분 권한 여부',
    '고용노동부·안전보건공단·지자체 등 공식 문의처에서 같은 명칭을 확인할 수 있는지'
  ];
}

function cautiousOfficialConclusion(analysis) {
  const institution = analysis?.institutionWords?.[0] || '해당 기관';
  if (institution.includes('고용노동부') || institution.includes('노동부')) {
    return '현재 확인된 공개 출처만으로는 고용노동부 정식 직원이나 공무원이라고 단정하기 어렵습니다. 다만 공고와 사업 자료 기준으로는 정식 근로감독관이라기보다 안전 관련 사업에 참여하는 지킴이/위촉·수행 인력 성격으로 보는 것이 안전합니다.';
  }
  return `현재 확인된 공개 출처만으로는 ${institution} 정식 소속이나 공적 신분이라고 단정하기 어렵습니다. 확인된 자료 기준으로는 관련 사업·제도에 참여하는 인력 성격인지 먼저 확인하는 것이 안전합니다.`;
}

function ensureEvidenceLimitedPhrasing(answer, analysis, researchPlan, sources) {
  const text = String(answer || '').trim();
  if (!text || !needsEvidenceLimitedOfficialPhrasing(analysis, researchPlan, sources)) return answer;
  if (/공개 출처(?:만으로는|를 충분히 확인하지 못해).*단정하기 어렵|현재 확인된 자료 기준|보는 것이 안전|개별 신분은 별도 확인/.test(text)) {
    return answer;
  }
  ...
  let softened = text;
  if (/1\.\s*결론\s*\n/.test(softened)) {
    const nextSection = softened.search(/\n\s*2\.\s*/);
    if (nextSection !== -1) {
      softened = softened.replace(/1\.\s*결론\s*\n[\s\S]*?(?=\n\s*2\.\s*)/, `1. 결론\n${cautiousConclusion}\n`);
    } else {
      softened = softened.replace(/1\.\s*결론\s*\n[\s\S]*/, `1. 결론\n${cautiousConclusion}`);
    }
  } else {
    softened = `현재 확인된 공개 출처만으로는 단정하기 어렵습니다. ${cautiousConclusion}\n\n${softened}`;
  }

  if (/5\.\s*지금 확인할 것\s*\n/.test(softened)) {
    const checkText = affiliationCheckText(analysis);
    softened = softened.replace(/5\.\s*지금 확인할 것\s*\n[\s\S]*/, `5. 지금 확인할 것\n${checkText}`);
  }
...
function affiliationCheckText(analysis) {
  return joinBullets(affiliationCheckList(analysis), '신분증, 공문, 명함, 위촉장, 소속기관명, 근로감독관 여부, 행정처분 권한 여부를 확인하세요.');
}

function synthesizeReviewedAnswer({ draftAnswer, review, sources, analysis, researchPlan }) {
  const sourceQuality = researchPlan?.sourceQuality || 'none';
  const weakSources = isWeakSourceQuality(sourceQuality);
  const evidenceLimitedOfficial = needsEvidenceLimitedOfficialPhrasing(analysis, researchPlan, sources);
  const caveat = weakSources ? '공개 출처를 충분히 확인하지 못해 일반적인 기준으로 정리했습니다. ' : '';
  const conclusion = evidenceLimitedOfficial
    ? cautiousOfficialConclusion(analysis)
    : buildReviewedConclusion(draftAnswer, analysis, weakSources);
  const displayedConclusion = (weakSources && conclusion.includes('공개 출처')) || evidenceLimitedOfficial ? conclusion : `${caveat}${conclusion}`;
  const reasonableInterpretation = buildReasonableInterpretation(draftAnswer, conclusion, analysis);
  ...
  const checkItems = [
    ...
    suggestedFixes.length ? '미확인 지점은 공식 안내나 담당 기관 답변으로 다시 확인하기' : ''
  ].filter(Boolean);
...
2. 확인된 내용
${sourceSummaryBullets(sources, sourceQuality, analysis).join('\n')}
...
- 평이한 한국어로 직접 답한다.
- 너무 길게 쓰지 않는다.
- 같은 섹션 제목이나 결론 표현을 본문에서 반복하지 않는다.
- “결론 아닙니다”, “확인된 내용은 확인되었습니다”, “합리적 해석은 합리적입니다”처럼 제목을 되풀이하는 어색한 문장을 쓰지 않는다.
- 사용자가 다음에 무엇을 해야 하는지 말한다.
- 공개 정보가 부족하거나 출처가 없으면 불확실성을 말한다.
- 출처가 뒷받침하는 것보다 강한 결론을 내리지 않는다.
- 공개/공식/소속/권한/신분 질문에서 출처가 사업·모집·공고만 확인하고 법적 신분은 직접 확인하지 못하면, “아니다”가 아니라 “공개 출처만으로는 단정하기 어렵다”라고 말한다.
- 확인된 사실은 출처에서 온 내용만 쓰고, 합리적 해석은 반드시 해석이라고 구분한다.
- 검토 메모나 AI 판단을 확인된 사실로 바꾸지 않는다.
...
복잡하거나 공식 근거가 필요한 질문은 다음 구조로 한 번만 답하고, 단순 질문은 필요한 만큼만 간단히 답한다:
...
function buildUserPrompt(question, sources, analysis, researchPlan) {
  ...
  const evidenceLimitContext = needsEvidenceLimitedOfficialPhrasing(analysis, researchPlan, sources)
    ? '\n\n근거 제한 규칙:\n- 이 질문은 공식/소속/권한/신분 관련 질문이며 현재 근거가 제한적입니다.\n- 출처가 사업·모집·공고를 보여주더라도 법적 신분을 직접 증명하지 않으면 “아니다/정식 직원이 아니다/공무원이 아니다”처럼 단정하지 마세요.\n- “현재 확인된 자료 기준으로는”, “공개 출처만으로는 단정하기 어렵습니다”, “~로 보는 것이 안전합니다”, “개별 신분은 별도 확인이 필요합니다” 중 하나 이상을 결론에 포함하세요.\n- 확인된 내용, 합리적 해석, 확인되지 않은 부분을 분리하세요.'
    : '';

  return `${questionContext}${strategyContext}${evidenceLimitContext}\n\n공개 검색 맥락:\n${buildSourceContext(sources)}\n\n위 정보만으로 자연스럽게 즉답해줘. 출처 목록 자체는 프론트에서 따로 보여주므로, 답변 본문에는 링크를 길게 나열하지 마.`;
}
...
async function buildAnswer(question, sources, usedSearch, analysis, hasEnoughEvidence = usedSearch, researchPlan, options = {}) {
  ...
  if (analysis?.needsSearch && !hasEnoughEvidence && !reviewRequired) {
    return {
      answer: ensureEvidenceLimitedPhrasing(
        buildCarefulFallbackAnswer(question, usedSearch, analysis, hasEnoughEvidence, researchPlan),
        analysis,
        researchPlan,
        sources
      ),
  ...
  if (!geminiDraft) {
    try {
      return {
        answer: ensureEvidenceLimitedPhrasing(
          await openaiAnswer(question, sources, usedSearch, analysis, researchPlan),
          analysis,
          researchPlan,
          sources
        ),
  ...
  if (reviewRequired && process.env.OPENAI_API_KEY) {
    try {
      ...
      return {
        answer: ensureEvidenceLimitedPhrasing(ensureSourceCaveat(geminiDraft, analysis, researchPlan), analysis, researchPlan, sources),
        reviewUsed: false,
        reviewExpected: reviewRequired,
        reviewFailed: true
      };
